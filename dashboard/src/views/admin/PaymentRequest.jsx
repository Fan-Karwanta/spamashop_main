  ...
  const Row = ({ index, style }) => {
    return (
      <div style={style} className='flex text-sm'>
        ...
        <div className='w-[25%] p-2 whitespace-nowrap'>
          <button disabled={loader} onClick={() => confirm_request(pendingWithdrows[index]?._id)} className='bg-green-500 shadow-lg hover:shadow-green-500/50 px-3 py-[2px] cursor-pointer text-white rounded-sm text-sm'>{(loader && paymentId === pendingWithdrows[index]?._id) ? 'loading..' : 'Confirm'}</button>
        </div>
      </div>
    )
  }
